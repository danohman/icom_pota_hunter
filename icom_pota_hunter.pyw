...
def get_spots():
    ...
    if len(spots) == 0:
        ...
        window['click_mark_worked'].update(visible=False)
...
def update_info_window(arr):
    ...
    window['click_mark_worked'].update(visible=True)
    
    for wrk in worked:
        if (wrk[0] == current_spot_frequency and wrk[1] == current_spot_activator):
            window['click_mark_worked'].update(visible=False)
            window['freq_call_worked'].update(visible=True)
...
col1 = sg.Col(
    [
        [
            ...
            sg.pin(sg.Button(' Mark Hunted ', key='click_mark_worked', button_color=('white','darkgreen'), visible=False)),          
            sg.Text(' (Hunted Frequency and Activator) ', key='freq_call_worked', visible=False, text_color='darkred', font=("Helvetica", 12, 'bold'))
        ],
        [
...
while True:
    ...
    if event == 'click_mark_worked':
        worked.append([current_spot_frequency, current_spot_activator])
